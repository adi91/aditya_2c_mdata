...
class repository_alfresco extends repository {
    ...
    public function __construct($repositoryid, $context = SYSCONTEXTID, $options = array()) {
        ...
        if (class_exists('SoapClient')) {
			...
			/***** added by pankaj wagh ********** 14/08/2012 **********/
            $this->username   = optional_param('al_username', $USER->username, PARAM_RAW);
            $this->password   = optional_param('al_password', $USER->al_password, PARAM_RAW);
			/***** added by pankaj wagh ********** 14/08/2012 **********/
            try{
    ...
    public function print_login() {
        ...
        if ($this->options['ajax']) {
			...
			$user_field->value  = $USER->username; /***** added by pankaj wagh ********** 14/08/2012 **********/
			...
			$passwd_field->value  = $USER->al_password; /***** added by pankaj wagh ********** 14/08/2012 **********/
            ...
            echo '<td><input type="text" name="al_username" value="'.$USER->username.'" /></td></tr>'; /***** added by pankaj wagh ********** 14/08/2012 **********/
            echo '<tr><td><label>'.get_string('password', 'repository_alfresco').'</label></td>';
            echo '<td><input type="password" name="al_password" value="'.$USER->al_password.'"/></td></tr>'; /***** added by pankaj wagh ********** 14/08/2012 **********/
            echo '</table>';
    ...
    public function search($search_text) {
	   ...
	   /* $nodes = $this->user_session->query($currentStore, $search_text);*/
	    /***** added by pankaj wagh ********** 14/08/2012 **********/		
		global $CFG;
		$arr_elements = $CFG->al_search_array;
		foreach ($arr_elements as $val)
		  {
				
				$arr[] = "@".$val.":\\\"".$search_text."\\\"";
		  }
		 
		 $searchtext = implode(" OR ",$arr)." NOT TYPE:\"cm:folder\"";
	   // $nodes =  $this->user_session->query($currentStore, "@cm\:Name:\"*html*\"");
	    $nodes =  $this->user_session->query($currentStore,  $searchtext);
		/***** added by pankaj wagh ********** 14/08/2012 **********/
        $ret = array();
        $ret['list'] = array();
       /***** added by pankaj wagh ********** 20/08/2012 **********/
		global  $OUTPUT;
        foreach($nodes as $v) {
            $ret['list'][] = array('title'=>$v->cm_name, 'source'=>$v->id,'thumbnail' => $OUTPUT->pix_url(file_extension_icon($v->cm_name, 32))->out(false));
        }
		/***** added by pankaj wagh ********** 20/08/2012 **********/
        return $ret;
    }
	public function my_search($search_text) {
        $space = optional_param('space', 'workspace://SpacesStore', PARAM_RAW);		
        //$currentStore = $this->user_session->getStoreFromString($space);
		//$repositoryUrl = 'http://localhost:8080/alfresco/api';
		//$repositoryUrl = 'http://205.251.113.162:8081/alfresco/api';
		//$repositoryUrl = 'http://223.4.174.44:8080/alfresco/api';
		$repositoryUrl = 'http://223.4.174.44:8080/alfresco/api';
		//$repositoryUrl = 'http://116.213.70.88:8080/alfresco/api';
		
		if(!($repository = new Alfresco_Repository($repositoryUrl))) die('failed to connect to repository');

		$moodle_ticket = false;
		foreach($_SESSION['SESSION'] as $key=>$value){
			if(preg_match('/alfresco_ticket_/',$key)){
				$ticket = $value;
				$moodle_ticket = true;
			}
		}
		if($moodle_ticket == false){
			$userName = 'admin';
			$password = 'Admin@123';
			if(!($ticket = $repository->authenticate($userName, $password))) die('failed to create a ticket to authenticate');
		}
		if(!($session = $repository->createSession($ticket))) die('could not create a session');
		$currentStore = $session->getStoreFromString($space);
       //print_r($currentStore);
	   /* $nodes = $this->user_session->query($currentStore, $search_text);*/
	    /***** added by pankaj wagh ********** 14/08/2012 **********/		
		global $CFG;//print_r($search_text);
		$arr_elements = $CFG->al_search_array;
		foreach ($arr_elements as $val)
		  {
		 ...
		 $searchtext = implode(" OR ",$arr)." NOT TYPE:\"cm:folder\"";
	   // $nodes =  $this->user_session->query($currentStore, "@cm\:Name:\"*html*\"");
	    $nodes =  $session->query($currentStore,  $searchtext);
       ...
       /***** added by pankaj wagh ********** 20/08/2012 **********/
        ...
        foreach($nodes as $v) {
			$strpath = '';
			$strfilename = '';
			$strfilename = $v->cm_name;
			if($v->id && "$v->cm_name" == "$search_text"){
				$strurl = '';
				$strurl = "http://223.4.174.44:8080/alfresco/download/direct/workspace/SpacesStore/";
				$strurl .= $v->id;
				$strurl .= "/";
				$strurl .= $v->cm_name;
				$strurl .= "?ticket=".$ticket;
				$str .= $strpath.",".$strfilename.",".$strurl."\r\n"; 
				$fp = fopen('filepathmap.csv', 'a');
				fwrite($fp,$str);
				fclose($fp);
				$ret[$v->cm_name] = $strurl;
			}else{
				$ret[$v->cm_name] = '';
			}
			//,'thumbnail' => $OUTPUT->pix_url(file_extension_icon($v->cm_name, 32))->out(false));
        }
		/***** added by pankaj wagh ********** 20/08/2012 **********/
		//print_r($ret);
        return $ret;
