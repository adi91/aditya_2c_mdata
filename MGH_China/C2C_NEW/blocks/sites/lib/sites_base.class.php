...
class sites_base {
    ...
    public function show() {
        ...
        $sort = optional_param('sort', 'site_id', PARAM_RAW);
		$dir  = optional_param('dir', 'ASC', PARAM_ALPHA);
		$page         = optional_param('page', 0, PARAM_INT);
		$perpage      = optional_param('perpage', 30, PARAM_INT); 
		 
        $sites = $DB->get_records('sites', null, $sort." ".$dir, '*', $page, $perpage);
		$sites_ = $DB->get_records('sites');		
		$baseurl = new moodle_url('index.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage));
		

        $this->outputs = $OUTPUT->paging_bar(count($sites_), $page, $perpage, $baseurl);

        if ($sites) {

            // Initialize table
            $table = $this->_create_sites_data_table(true,$sort,$dir,$perpage);
            
            foreach ($sites as $site) {

            	// Preset actions
                $editlink = $CFG->wwwroot.'/blocks/sites/index.php?action=edit&id='.$site->id;
                $deletelink = $CFG->wwwroot.'/blocks/sites/index.php?action=delete&id='.$site->id;


                $actions = array();
                $actions[] = html_writer::link($editlink, " <img src=\"" . $OUTPUT->pix_url('t/edit') . "\" alt=\"\" />");
                $actions[] = html_writer::link($deletelink, " <img src=\"" . $OUTPUT->pix_url('t/delete' ) . "\" alt=\"\" />");
                
            	if ($site->site_created) {
                	$timecreatedstring = userdate($site->site_created);
                } else {
                	$timecreatedstring = '';
                }
                
            if ($site->site_modified) {
                	$timemodifiedstring = userdate($site->site_modified);
                } else {
                	$timemodifiedstring = '';
                }
                ...
                // Populate table
                $table->data[] = array(format_text($site->id, FORMAT_HTML),
                	format_text($site->site_id, FORMAT_HTML),
                	format_text($site->site_name, FORMAT_PLAIN),    
                  //  format_text($site->site_desc, FORMAT_PLAIN),
                    format_text($site->site_author, FORMAT_PLAIN),
                    format_text($timecreatedstring, FORMAT_PLAIN),
                    format_text($timemodifiedstring, FORMAT_PLAIN),
                    ''.implode(' ', $actions).'');
            }
        ...
        // If there aren't presets notify it
            ...
            $createlink = '<a href="'.$CFG->wwwroot.'/blocks/sites/index.php?action=create">'.strtolower(get_string("actioncreate", "block_sites")).'</a>';
            ...
            $this->outputs .= '<ul>'.get_string('nosites', 'block_sites');
            $this->outputs .= '<li>'.get_string('tocreateclick', 'block_sites', $createlink).'</li>';
    ...
    public function log() {
        ...
        if ($this->mode != 'show') {
            ...
            add_to_log($SITE->id, 'block_sites', $action, '', $this->id);
        }
    }
    ...
    protected function _display_header() {
        ...
        // Strings
        $actionstr = get_string('action'.$this->action, 'block_sites');
        $modestr = get_string($this->action.$this->mode, 'block_sites');
        $titlestr = get_string('pluginname', 'block_sites');

        // Header
        $PAGE->set_title($titlestr);
        $PAGE->set_heading($SITE->fullname);

        $PAGE->navbar->add(get_string('pluginname', 'block_sites'),
            new moodle_url($CFG->wwwroot.'/blocks/sites/index.php'));
     ...
     * @return html_table
     */
    protected function _create_sites_data_table($actionstable = true,$sort='site_id',$dir='ASC',$perpage=30) 
	{
		global $OUTPUT;
        $table = new html_table();
        $table->attributes['class'] = 'generaltable boxaligncenter sites_table';
        $table->align = array('left', 'left',  'left', 'center', 'center', 'center');
       /* $table->head  = array(get_string('siteid', 'block_sites'), get_string('name'),get_string('description'),
            get_string('siteauthor', 'block_sites'),  
            get_string('sitecreated', 'block_sites'), get_string('sitemodified', 'block_sites'));*/
			
	 $columns = array("site_fk","site_id","site_name","site_author","site_created","site_modified");
	   foreach ($columns as $column) {
        $string[$column] = get_string("$column", 'block_sites');
        if ($sort != $column) {
            $columnicon = "";
	 ...
	 $table->head  = array($site_fk,$site_id,$site_name,$site_author,$site_created,$site_modified);	


        if ($actionstable) {
            $table->align[] = 'left';
            $table->head[] = get_string('actions');
            $table->size = array('10%',  '27%', '10%', '15%', '15%', '8%');
        } else {
        	$table->size = array('10%', '30%', '10%', '20%', '20%', '5%');
        }
