...
class course_enrolment_manager {
    ...
    public function get_total_users() {
        ...
        if ($this->totalusers === null) {
        	...
        	/*Swapnil to add the siteid  for in query 02/04/12 */
        	if(!is_siteadmin($USER->id,SITE_ADMIN_ROLE))
            {
           		// $sqltotal .= " WHERE u.site_fk=:sites";
           		// $params['sites'] = $USER->site_fk;
				
				/*  following code modyfied by chandra to add multiple center funtionality for non site admin users */
				/* .... start... */
				if($USER->site_fk){
					$site_fk = str_replace("*",",",$USER->site_fk);
					$sqltotal .= " WHERE u.site_fk IN (".$site_fk.")";
					$sqltotal .= " AND u.deleted=0";
				}
				/* .... ends... */
            }
    ...
    public function get_users($sort, $direction='ASC', $page=0, $perpage=25) {
        ...
        if (!array_key_exists($key, $this->users)) {
            ...
            $sql = "SELECT DISTINCT $ufields, ul.timeaccess AS lastseen
             ...
             /* Swapnil To get the site wise student  02/04/12 */
            if(!is_siteadmin($USER->id))
            {
           		 //$sql .= " WHERE u.site_fk=:sites";
           		 //$params['sites'] = $USER->site_fk;
				
				/* code modyfied by chandra for implementing multiple center functionality */
				...
				if($USER->site_fk){
					$site_fk = str_replace("*",",",$USER->site_fk);
					$sql .= "WHERE  u.site_fk IN (".$site_fk.")";
					$sql .= " AND u.deleted=0 ";
				}
    ...
    public function get_potential_users($enrolid, $search='', $searchanywhere=false, $page=0, $perpage=25) {
        ...
        $sql = " FROM {user} u
                WHERE $wherecondition
                      AND u.deleted = 0 AND u.id NOT IN (SELECT ue.userid
                                         FROM {user_enrolments} ue
                                         JOIN {enrol} e ON (e.id = ue.enrolid AND e.id = :enrolid))";
        /* Swapnil To get the site wise student  02/04/12 */
            //if(!is_siteadmin($USER->id))
            //{
           		 //$sql .= " AND u.site_fk=:sites";
           		 //$params['sites'] = $USER->site_fk;
				 
				 /* code modyfied by chandra for implementing multiple center functionality */
				
				/* .... starts ...... */
                $center = $DB->get_record('sites',array('site_centerid'=>$course->category));				
				$sql .= " AND (u.site_fk like '".$center->id."' OR u.site_fk like '%*".$center->id."' OR u.site_fk like '".$center->id."*%') ";				
				/* .... ends ...... */
            //}	
        $sql .= " AND u.deleted = 0 ";  
