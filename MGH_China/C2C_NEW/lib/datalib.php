...
function get_users($get=true, $search='', $confirmed=false, array $exceptions=null, $sort='firstname ASC',
    ...
    $check_for_center_admin_default_center = true;
    ...
    if ($extraselect) {
		...
		foreach($params as $param){
			...
			if(preg_match('/site_fk=:ex_simpleselect/',$extraselect)){
				$check_for_center_admin_default_center = false;
				$extraselects = explode(" AND ",$extraselect);
				foreach($extraselects as $extraselect1){
					if(preg_match('/site_fk=:ex_simpleselect/',$extraselect1)){
						$param_val = explode(":",$extraselect1);
						//echo '<pre>';print_r($extraselect1);echo '</pre>';echo '<pre>';print_r($params[$param_val[1]]);echo '</pre>';
						$pattern = array("/$extraselect1/");
						$replacement = array('(site_fk like "%'.$params[$param_val[1]].'%" or site_fk like "%*'.$params[$param_val[1]].'%" or site_fk like "%'.$params[$param_val[1]].'*%" or site_fk like "%*'.$params[$param_val[1]].'*%")');
						$extraselect = preg_replace($pattern,$replacement,$extraselect);
					}
				}
			}
			$c++;
	...
	/*Swapnil -- to get user as per the siteid for login user 2/04/12*/
    if($check_for_center_admin_default_center && !is_siteadmin($USER->id) && !is_mhescordinator())
	{
		 $select .= " AND ";
		 if($USER->site_fk){
			...
			foreach($user_sites as $user_site){
				$select .= " (site_fk like '".$user_site."' OR site_fk like '%*".$user_site."' OR site_fk like '".$user_site."*%' OR site_fk like '%*".$user_site."*%')";
				if($c < (count($user_sites)-1)){
					$select .= " OR ";
				}
				$c++;
			}
...
function get_users_listing($sort='lastaccess', $dir='ASC', $page=0, $recordsperpage=0,
    ...
    return $DB->get_records_sql("SELECT id, username, email, firstname, lastname, city, country,
                                        lastaccess, confirmed, mnethostid, suspended,site_fk,parentlist $extrafields
                                   FROM {user}
                                  WHERE $select
