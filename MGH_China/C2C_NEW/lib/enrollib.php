...
function apt_enrol_get_users_courses($userid, $onlyactive = false, $fields = NULL, $sort = 'visible DESC,sortorder ASC',$searchname) {
    global $DB;
    ...
    $basefields = array('id', 'category', 'sortorder','format',
                        'shortname', 'fullname', 'idnumber',
                        'startdate', 'visible',
                        'groupmode', 'groupmodeforce');
    ...
    if ($onlyactive) {
       // $subwhere = "WHERE ue.status = :active AND e.status = :enabled AND ue.timestart < :now1 AND (ue.timeend = 0 OR ue.timeend > :now2)";
        $subwhere = " WHERE ue.status = :active AND e.status = :enabled  AND (ue.timeend = 0 OR ue.timeend > :now2)";
       // $params['now1']    = round(time(), -2); // improves db caching
        $params['now2']    = $params['now1'];
        $params['active']  = ENROL_USER_ACTIVE;
        $params['enabled'] = ENROL_INSTANCE_ENABLED;
    ...
    $sql = "SELECT $coursefields $ccselect
              ...
              JOIN {groups_members} gm ON ( gm.groupid = g.id AND gm.userid = :userid1)
              JOIN (SELECT DISTINCT e.courseid
                      FROM {enrol} e
                      JOIN {user_enrolments} ue ON (ue.enrolid = e.id AND ue.userid = :userid)
                 $subwhere
    ...
    //Harshal: #458
    if(is_student())
    	$sql .= "AND g.startdate < :now1 ";
           /*removed as the feature course are not shown for as class start date is  techare and student */
           //AND g.startdate < :now1 
      if(trim($searchname) !='')
     $sql .= " and c.fullname like '%".trim($searchname)."%'";
