...
function require_login($courseorid = NULL, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = false) {
    ...
    if ($course->id == SITEID) {
        ...
        {
        	//this is added to check whether the student is parent or not .
        	//if parent then course access will be given for view
        	$access = true;
        } 
...
/**
 * function to get list for site  24/04/12 */
...
function  get_site_name() {
	...
	{
		$sites = str_replace("*",",",$USER->site_fk);
		$sql .= "  where  id in (".$sites.")";
		//$params['sites']= explode("*",$USER->site_fk);
	}
...
function  get_site_name_by_id($sid) {
	global $DB, $USER;
	$sid = explode("*",$sid);
	$sid = implode(",",$sid);
	$params =array();
...
function is_parent(){
    global $CGF,$DB,$USER;
	$query1 = "SELECT user.id as userid FROM `mdl_user` as user left join `mdl_user` as parent on user.username = parent.parentlist WHERE user.deleted = 0 and user.parentlist like  '".$USER->username."'";
	$params = array();
...
function is_teacher(){
    global $CGF,$DB,$USER;
	$teacher = $DB->get_records('role_assignments', array('userid'=>$USER->id,'roleid'=>'3'));
	if($teacher){
...
function is_non_editing_teacher(){
    global $CGF,$DB,$USER;
	$teacher = $DB->get_records('role_assignments', array('userid'=>$USER->id,'roleid'=>'4'));
	if($teacher){
	    $_SESSION['user_role'] = 'non_editing_teacher';
		return true;
	}else{
		return false;
	}
}
function is_centeradmin(){
    global $CGF,$DB,$USER;
	
	$centeradmin = $DB->get_records('role_assignments', array('userid'=>$USER->id,'roleid'=>'10'));
	if($centeradmin){
	    $_SESSION['user_role'] = 'centeradmin';
		//print_r('User Role'.$_SESSION['user_role']);
		return true;
	}else{
		return false;
	}
	...
	foreach($categories as $category){
		...
		//echo '<pre>';print_r($categorycontext->id);echo '</pre>';
		$role_assignments = $DB->get_records('role_assignments', array('contextid'=>$categorycontext->id,'userid'=>$USER->id));
...
function is_institute_cordinator(){
    global $CGF,$DB,$USER;
	
	$institute_cordinator = $DB->get_records('role_assignments', array('userid'=>$USER->id,'roleid'=>'13'));
	if($institute_cordinator){
	    $_SESSION['user_role'] = 'institute_cordinator';
		//print_r('User Role'.$_SESSION['user_role']);
		return true;
	}else{
		return false;
...
function is_mhescordinator(){
    global $CGF,$DB,$USER;
	
	$mhescordinator = $DB->get_records('role_assignments', array('userid'=>$USER->id,'roleid'=>'12','contextid'=>'1'));
	if($mhescordinator){
	    $_SESSION['user_role'] = 'mhescordinator';
		return true;
	}else{
		return false;
	}
...
function is_student(){
    global $CGF,$DB,$USER;
	$student = $DB->get_records('role_assignments', array('userid'=>$USER->id,'roleid'=>'5'));
	if($student){
...
function get_mychildren(){
    if(is_parent()){
		global $CGF,$DB,$USER;
		$query1 = "SELECT user.* FROM `mdl_user` as user left join `mdl_user` as parent on user.username = parent.parentlist WHERE user.deleted = 0 and user.parentlist like  '".$USER->username."'";
		$params = array();
		$students = $DB->get_records_sql($query1,$params);
		if($students){
			return $students;
...
function is_currentlsection($sectionid){

	$sect_name = $DB->get_record('course_sections', array('id'=>$sectionid));
	
	if($sect_name->startdate){
		$time = time(); 
		$currenttime = explode(':',userdate($time, '%H:%M:%S:%m:%d:%Y')); 
		$currentimestamp = time("d m Y h:m:s", mktime($currenttime[0], $currenttime[1], $currenttime[2], $currenttime[3], $currenttime[4], $currenttime[5]));
		$timediff = $sect_name->startdate - $currentimestamp;			
		if($timediff < 0){
		}else{
		   $days = floor($timediff / (60 * 60 * 24)); // extract hours 
		   $divisor_for_hours = $timediff % (60 * 60 * 24); 
		   $hours = floor( $divisor_for_hours / (60 * 60));// extract minutes
		   $divisor_for_minutes = $timediff % (60 * 60);
		   $minutes = floor($divisor_for_minutes / 60);// extract the remaining seconds
		   $divisor_for_seconds = $divisor_for_minutes % 60;
		   $seconds = ceil($divisor_for_seconds);// return the final array
		   $obj = array(
				"d" => (int) $days,
				"h" => (int) $hours,
				"m" => (int) $minutes,
...
function get_schedule($startdate = NULL){
	if($startdate == NULL){
	  $currenttime = time();	  
	}else{	
	...
	for($i=0;$i<80;$i++){
		$lessonstartdate['time'][($i+1)] = $currenttime;	  
		$lessonstartdate['systemformatted'][($i+1)] = date('d M Y h:m a',$lessonstartdate['time'][($i+1)] );
		if($i%2 != 0){
			$currenttime = ($currenttime + (7*24*60*60));
		}	
	}
	return $lessonstartdate['time'];
}
function schedule_lessons($courseid){
    global $DB,$CFG,$USER;
	
	$course = $DB->get_record('course', array('id'=>$courseid));
	$startdate = $course->startdate;
	$lessonschedule = get_schedule($startdate);
	
	$select = "course = ".$courseid." AND section <> 0";
	$course_sections = $DB->get_records_select('course_sections', $select);
	...
	foreach($course_sections as $course_section){
		...
		$data->name = 'Lession '.($section_count+1);
		$data->startdate = $lessonschedule[($section_count+1)];
		
		$DB->update_record_raw('course_sections', $data);
		
		$section_count++;
	}	
}
function timestamp_formatting($time){
	...
	$min = floor($time/60);
	$sec = $time-($min*60);
	$time = $days.' days '.$hour.' hours '.$min.' minutes '.$sec.' seconds ';
	return $time;
}


function get_topic_completion_percentage($courseid,$section,$userid)
{
	global $DB;
	$activities_completion_percentage= 0;
	$section_arr = $DB->get_record("course_sections",array('course'=>$courseid,'id'=>$section));
	$group_act=groups_get_groupby_role($userid,$courseid);
	$sec_act_group=get_group_activity($group_act->id,$section,$courseid);
	
	if($section_arr->sequence =="" || empty($section_arr->sequence) || is_null($section_arr->sequence) )
	{
		return $activities_completion_percentage;
	}
	else 
	{
		$activities = explode(',',$section_arr->sequence);
		...
		for($i=0;$i<$count;$i++){
			if($activities[$i] !="")
			{
			 /* $completionquery = 'SELECT module_completion.completionstate 
			   FROM {course_modules}  as modules  
			   left join {course_modules_completion} as module_completion 
			   on modules.id =  module_completion.coursemoduleid 
			   where modules.course = '.$courseid.' 
			   and modules.section = '.$section.' 
			   and modules.id = '.$activities[$i].' 																   
			   and module_completion.userid = '.$userid;
			   $completion = $DB->get_record_sql($completionquery,array());
		...
		return $activities_completion_percentage = number_format((($completioncount/$count)*100),0);

	}
}

function get_topic_completion_activity($courseid,$section,$userid)
{
	global $DB;
	
	$activities_completion=array();
	$activity_com="";
	$section_arr = $DB->get_record("course_sections",array('course'=>$courseid,'id'=>$section));
	$rawmods = get_course_mods($courseid);
	$group_act=groups_get_groupby_role($userid,$courseid);
	$sec_act_group=get_group_activity($group_act->id,$section,$courseid);
	$get_act_record_count=get_group_act_no($group_act->id,$courseid);
	$startday=strtotime('0 day', strtotime(date('Y-m-d')));
	if($section_arr->sequence =="" || empty($section_arr->sequence) || is_null($section_arr->sequence))
	{
		 	$activities_completion['count']=0;
			$activities_completion['percentage']=0;
			return $activities_completion;
	}
	else 
	{
		$activities = explode(',',$section_arr->sequence);
		...
		$totCount=0;
		for($i=0;$i<$count;$i++){
			if($activities[$i] !="" && $rawmods[$activities[$i]]->visible==true)
			{
			 /* $completionquery = 'SELECT module_completion.completionstate 
			   FROM {course_modules}  as modules  
			   left join {course_modules_completion} as module_completion 
			   ...
			   and module_completion.userid = '.$userid;
			   $completion = $DB->get_record_sql($completionquery,array());*/
				$sec_class_act= get_group_les_sin_activity($group_act->id,$section,$courseid,$activities[$i]);
				if($sec_class_act->availablefrom < $startday && $get_act_record_count>1){
					$completioncount++;
					$activity_com .='<span class="edu-icon_01"></span>';																      
				   }else
				   {
				   	$not_com++;
					$activity_com .='<span class="edu-icon_02"></span>';	
				 }
		    }
		}
	$totCount= $completioncount + $not_com;
	$activities_completion['percentage'] = number_format((($completioncount/$totCount)*100),0);
	$activities_completion['count'] = $totCount;
	$activities_completion['compcount'] = $completioncount;
	$activities_completion['act'] = $activity_com;
	}
	
	return $activities_completion;
}

function get_activity_completion_mod($id)
{
	global $DB;
	$complition= $DB->get_record("course_modules_completion",array('coursemoduleid'=>$id));
	return $complition->completionstate;
}


function get_course_completion($courseid,$groupid)
{
	global $DB;
		$classmembers = $DB->get_records('groups_members',array('groupid'=>$groupid));
	
		$startday=strtotime('0 day', strtotime(date('Y-m-d')));
		$sql_completion ="SELECT (SELECT COUNT(DISTINCT mca1.sectionno) FROM {class_activity} AS mca1  WHERE section IS NOT NULL AND mca.course = mca1.course 
		AND mca.groupid = mca1.groupid AND mca1.availablefrom < ? ) AS completionsetion ,
		COUNT(DISTINCT sectionno)  AS noofsection FROM {class_activity} AS mca WHERE mca.section IS NOT NULL AND mca.course = ? AND mca.groupid = ?";
		$rs_completion = $DB->get_record_sql($sql_completion,Array($startday,$courseid,$groupid));
		$per = round(($rs_completion->completionsetion/$rs_completion->noofsection)*100,0);
		if($per >0)
		{
			return $per;
		}
...
function get_course_lesson_active($groupid,$availabletime)
{
	
	global $DB;
	
	$sql ="SELECT * FROM mdl_groups g
			LEFT JOIN mdl_class_activity  ca ON ca.groupid= g.id
			WHERE g.id=? AND ca.availablefrom <= ?
			GROUP BY ca.sectionno DESC LIMIT 0,1";
	
	$param = array($groupid,$availabletime);
	
	$res = $DB->get_record_sql($sql,$param);
		
		$datevalue= date("l",$availabletime);
		$sql ="SELECT * FROM mdl_groups WHERE id=? AND weekday1=?";
		$param = array($groupid,$datevalue);
		$res_field= $DB->get_record_sql($sql,$param);
		if($res_field)
		{
			$weekday2=$res->starttime; 
		}
		else
		{
			$weekday2=$res->starttime2;
		}
		if(is_mhescordinator()){
			$set_weekday1=date("H:i a", $res->availablefrom);
		}else{
			$set_weekday1=date("D", $res->availablefrom)."<br><br>".date("H:i", $res->availablefrom);
		}
		return $set_weekday1;
}
function quiz_report(){
	global $DB,$USER;
	$params = array();
	$sql_select_courses = "SELECT instanceid from {context} where id in (SELECT contextid from {role_assignments} where roleid = 5 and userid = $USER->id)";
	$sql_select_quizzes = "SELECT id from {quiz} where course in ($sql_select_courses)";
	$sql_select_quiz_grades = "SELECT * from {quiz_grades} where quiz in ($sql_select_quizzes) and userid = $USER->id";
	$grade_results = $DB->get_records_sql($sql_select_quiz_grades,$params);

	$str = '';
	$str .= '<div  class="scoreboard">';
    $gradecount = 1;
	foreach($grade_results as $grade_result){
		$quiz = $DB->get_record('quiz',array('id'=>$grade_result->quiz));
		
		$sql_select_quiz_attempt = "
		SELECT id,uniqueid,max(quiz_attempt.sumgrades) 
		from {quiz_attempts} quiz_attempt 
		where quiz_attempt.quiz = $grade_result->quiz and quiz_attempt.userid = $USER->id
		";
		$attempt_results = $DB->get_record_sql($sql_select_quiz_attempt,$params);
		$sql_select_question_attempts = "
		SELECT * FROM {question_attempts} question_attempts left join {question_attempt_steps} question_attempt_steps on question_attempts.id = question_attempt_steps.questionattemptid left join {question} question on question_attempts.questionid = question.id where question_attempts.questionusageid = $attempt_results->uniqueid and question_attempt_steps.sequencenumber = 2
		";
		$question_attempt_results = $DB->get_records_sql($sql_select_question_attempts,$params);
		
		$category_grade = array();
		$c=0;
		foreach($question_attempt_results as $question_attempt_result){
			$category_grades[$question_attempt_result->category]['maxmark'][$question_attempt_result->questionid]= $question_attempt_result->maxmark;
			$category_grades[$question_attempt_result->category]['marksobtained'][$question_attempt_result->questionid] = $question_attempt_result->fraction;
			$category_grades[$question_attempt_result->category]['category'] = $question_attempt_result->category;
			$c++;
		}		
		$c=1;
		if(($gradecount%2) == 1){
		...
		$str .= '<dl id="'.$dl_class.'">';
		$str .= '<dt>'.$quiz->name.' : <span class="totnum1marks">'.intval($grade_result->grade).'%</span></dt>';
		foreach($category_grades as $category_grade){
			$cat_name = $DB->get_record('question_categories',array('id'=>$category_grade['category']));
			$cat_grade[$cat_name->name]=((array_sum($category_grade['marksobtained'])/array_sum($category_grade['maxmark']))*100);
			$str .= '<dd><span class="num1">'.$c.'</span>'.$cat_name->name.' <span class="num1marks">'.intval($cat_grade[$cat_name->name]).'%</span></dd>';
			$c++;
		}
		$str .= '</dl>';
		$gradecount++;
...
function pre_post_quiz_report(){
	global $DB,$USER;
	$params = array();
	$sql_select_courses = "
	SELECT instanceid from {context} context
	left join {course} course on context.instanceid = course.id 
	where course.shortname = 'preassessment' 
	OR course.shortname = 'postassessment' 
	AND	context.id in (SELECT contextid from {role_assignments} where roleid = 5 and userid = $USER->id)";
	
	$sql_select_courses_str = "";
	$searchcourse = '';
   $mycourses = apt_enrol_get_users_courses($USER->id,true,'*', 'visible DESC,sortorder ASC',$searchcourse);
   foreach($mycourses as $mycourse){
		if(strtolower($mycourse->shortname) == 'pre assessment' || strtolower($mycourse->shortname) == 'preassessment' || strtolower($mycourse->shortname) == 'post assessment' || strtolower($mycourse->shortname) == 'postassessment'){
			$sql_select_courses_str .= $mycourse->id.',';
		}
   }
   $sql_select_courses = substr($sql_select_courses_str,0,-1);
   //echo $sql_select_courses;
   if($sql_select_courses != ''){
	$sql_select_quizzes = "SELECT id from {quiz} where course in ($sql_select_courses)";
	$sql_select_quiz_grades = "SELECT * from {quiz_grades} where quiz in ($sql_select_quizzes) and userid = $USER->id";
	//echo $sql_select_quiz_grades;
	//die();
	$grade_results = $DB->get_records_sql($sql_select_quiz_grades,$params);
...
function lesson_quiz_report($courseid,$sectionid,$moduleid){
	global $DB,$USER;
	$params = array();
	$sql_select_quizzes = "SELECT instance from {course_modules} where course = $courseid AND module = $moduleid AND section = $sectionid";
	$sql_select_quiz_grades = "SELECT * from {quiz_grades} where quiz in ($sql_select_quizzes) and userid = $USER->id";
	//echo $sql_select_quiz_grades;
	//die();
	$grade_results = $DB->get_records_sql($sql_select_quiz_grades,$params);
	...
	echo '<div style="float:left;"><a href="#section-'.$section->section.'" class="buttonstyled" onclick="showasses_report('.$sectionid.');">Assessments Results</a></div>';			
	$str = '';
	$str .= '<div  class="scoreboard">';
    $gradecount = 1;
	foreach($grade_results as $grade_result){
...
function user_quiz_report($courseid,$sectionid,$moduleid,$userid=''){
	global $DB,$USER;
	if($userid == ''){
		$userid = $USER->id;
	}
	$params = array();
	$sql_select_quizzes = "SELECT instance from {course_modules} where course = $courseid AND module = $moduleid AND section = $sectionid";
	$sql_select_quiz_grades = "SELECT * from {quiz_grades} where quiz in ($sql_select_quizzes) and userid = $userid";
	//echo $sql_select_quiz_grades;
	//die();
	$grade_results = $DB->get_records_sql($sql_select_quiz_grades,$params);
    if($grade_results){
	echo '<div style="float:left;"><a href="#" class="buttonstyled" onclick="showasses_report('.$sectionid.');">Assessments Results</a></div>';			
	$str = '';
	$str .= '<div  class="scoreboard">';
    $gradecount = 1;
	foreach($grade_results as $grade_result){
...
function user_quiz_attempt($attempt){
	global $DB,$USER;
	$str .= '<div  id = "categorized_attempt_report_'.$attempt.'" style="display:none;"  class="scoreboard">';
	$sql_select_quiz_attempt = "
		SELECT id,uniqueid,max(quiz_attempt.sumgrades) 
		from {quiz_attempts} quiz_attempt 
		where quiz_attempt.id = $attempt
		";
		$attempt_results = $DB->get_record_sql($sql_select_quiz_attempt,$params);
		$sql_select_question_attempts = "
		SELECT * FROM {question_attempts} question_attempts left join {question_attempt_steps} question_attempt_steps on question_attempts.id = question_attempt_steps.questionattemptid left join {question} question on question_attempts.questionid = question.id where question_attempts.questionusageid = $attempt_results->uniqueid and question_attempt_steps.sequencenumber = 2
