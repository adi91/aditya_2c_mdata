...
class user_filtering {
    ...
    function user_filtering($fieldnames=null, $baseurl=null, $extraparams=null) {
		...
		/*Swapnil: added filter for site_fk  2/4/12 */
        if (empty($fieldnames)) {
            $fieldnames = array('realname'=>0, 'lastname'=>1, 'firstname'=>1, 'email'=>1, 'city'=>1, 'country'=>1,
                                ...
                                'nevermodified'=>1, 'username'=>1, 'auth'=>1, 'mnethostid'=>1, 'site_fk'=>1);
        }
    ...
    function get_field($fieldname, $advanced) {
        ...
        switch ($fieldname) {
            ...
            case 'mnethostid':
                ...
                /*Swapnil: site filter added 2/04/12 */ 
	            	...
	            	$choices = array();
		       		foreach ($sites as $site) {
						if(is_siteadmin() || is_mhescordinator()){
		                      $choices[$site->id] = $site->site_name;
						}else{
							if(in_array($site->id,explode("*",$USER->site_fk))){
								$choices[$site->id] = $site->site_name;
							}
						}
		             }
			//print_r($choices);	
            return new user_filter_simpleselect('site', get_string('site_filter','block_sites'), $advanced, 'site_fk', $choices);
