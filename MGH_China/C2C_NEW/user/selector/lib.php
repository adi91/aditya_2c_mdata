    ...
    protected function output_optgroup($groupname, $users, $select) {
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                ...
                $output .= '    <option' . $attributes . ' value="' . $user->id . '"  title="'.$this->output_user($user).'" alt="'.$this->output_user($user).'">' .
...
class group_non_members_selector extends groups_user_selector_base {
    ...
    public function find_users($search) {
        ...
        $fields = "SELECT r.id AS roleid, r.shortname AS roleshortname, r.name AS rolename, u.id AS userid,
                          ...
                          IF(roleid = 5,(SELECT count(igm.groupid)
                             FROM {groups_members} igm
                             JOIN {groups} ig ON igm.groupid = ig.id
                            WHERE igm.userid = u.id AND ig.courseid = :courseid),0) AS numgroups";
         $sql = "   FROM {user} u
              ...
              LEFT JOIN {role} r ON r.id = ra.roleid
                  WHERE u.deleted = 0
                        ...
                        AND $searchcondition  ";
        $orderby = " HAVING numgroups =0 ORDER BY u.lastname, u.firstname";
