...
function useredit_shared_definition(&$mform, $editoroptions = null) {
    ...
    //$mform->setType('institution', PARAM_MULTILANG);
	 ...
	 $mform->addElement('hidden', 'institution', $user->site_fk);
     $mform->setType('institution', PARAM_MULTILANG);
	...
	$sql = "SELECT * FROM {user} u where deleted = 0 ";
    $params = '';
    $rs = $DB->get_records_sql($sql, array('deleted'=>0));
	?><script language="javascript">
		function adduser(username){
			var user = username;
			
			 if(document.getElementById("id_parentlist").value){
			    var str = document.getElementById("id_parentlist").value;
				
				if(str.indexOf(user+'*') != -1){
				   //alert(str.indexOf(id+'*'));
				}else{
					document.getElementById("id_parentlist").value = document.getElementById("id_parentlist").value + '*' + user;
				}
			  }else{
				document.getElementById("id_parentlist").value = user;
			}
		...
		function deleteuser(username){
			var user = username;
			 ...
			 if(document.getElementById("id_parentlist").value){
				...
				if(str.indexOf('*'+user) != -1){
				  document.getElementById("id_parentlist").value = str.replace('*'+user,"");
		...
		function display_parent(flag){
		   if(flag == 'show'){
		       document.getElementById("moodle_Parents").style.display = 'block';
		   }else{
				document.getElementById("moodle_Parents").style.display = 'none';
				document.getElementById("id_parentlist").value = "";
		   }
		}
	</script><?php
	$current_userid = optional_param('id', $USER->id, PARAM_INT);
	if ($current_userid != -1 && !$current_user = $DB->get_record('user', array('id'=>$current_userid))) {
		print_error('invaliduserid');
    }
	if($current_userid != -1){
	$currentparent_list = explode('*',$current_user->parentlist);
	
	}
	if(is_siteadmin() || is_centeradmin()){
	
	$mform->addElement('header', 'moodle_studentcheck', get_string('student_check'));
	$mform->addElement('checkbox', 'isstudent', null, get_string('student_check_text'), array('onclick' => 'if(this.checked){display_parent("show")}else{display_parent("hide")}'));
				
	$mform->addElement('header', 'moodle_Parents', get_string('parent'));
	
	$mform->addElement('static', 'parentinfo', '', get_string('parentinfo'));
	
	 foreach($rs as $us){ 
	    $has_any_role_assigned = $DB->get_records('role_assignments',array('userid'=>$us->id));
		if(!$has_any_role_assigned && !is_siteadmin($us->id) && !isguestuser($us->id)){
			if(is_centeradmin()){
				$uploader_centers = explode("*",$USER->site_fk);
				$user_centers = explode("*",$us->site_fk);
				//echo '<pre>';print_r($uploader_centers);echo '</pre>';
				//echo '<pre>';print_r($user_centers);echo '</pre>';
				//echo '<pre>';print_r(array_intersect($user_centers,$uploader_centers));echo '</pre>';
				//echo '<pre>';print_r(count(array_intersect($user_centers,$uploader_centers)));echo '</pre>';
				if(count(array_intersect($user_centers,$uploader_centers))){
					$mform->addElement('checkbox', 'user'.$us->id, null, $us->firstname.' '.$us->lastname, array('onclick' => 'if(this.checked){adduser("'.$us->username.'")}else{deleteuser("'.$us->username.'")}'));
					if(in_array($us->username,$currentparent_list)){
						$mform->setdefault('user'.$us->id,true);
					}
				}
			}elseif(is_siteadmin()){
				$mform->addElement('checkbox', 'user'.$us->id, null, $us->firstname.' '.$us->lastname, array('onclick' => 'if(this.checked){adduser("'.$us->username.'")}else{deleteuser("'.$us->username.'")}'));
	...
	$mform->addElement('text', 'parentlist', get_string('parentlist'), 'maxlength="30" size="25"');
