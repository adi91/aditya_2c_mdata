    ...
    for($day = 1; $day <= $display->maxdays; ++$day, ++$dayweek) {
       ...
       if(isset($eventsbyday[$day])) {
           ...
           // $cell = '<a href="'.(string)$dayhref.'" '.$popup.'>'.$day.'</a>';
			...
			global $DB;
			if(is_siteadmin() || is_mhescordinator() || is_institute_cordinator())
			{;
					$param = array();
					if(is_institute_cordinator()){
						...
						//echo $catid;
						$course_sql = "SELECT id from {course} where category in ($catid)";
						$courses = $DB->get_records_sql($course_sql);
						foreach($courses as $course){
							$courseid .= $course->id.',';
						}
						$courseid1 = substr($courseid,0,-1);						
						$event_sql = "SELECT * FROM {class_activity} where course in ($courseid1)";
						$class_events = $DB->get_records_sql($event_sql);
					}else{
						$class_events = $DB->get_records('class_activity',array());		
					}
					$calendardate = strtotime($day.'-'.$m.'-'.$y.' 00:00:00');
					$calenderenddate= strtotime($day.'-'.$m.'-'.$y.' 23:59:59');
					$is_link = false;
					foreach($class_events as $class_event){
						if(!is_null($class_event->section)) {
						   if($class_event->availablefrom  >= $calendardate && $class_event->availablefrom  < $calenderenddate ){
								$class .= ' calendar_event_course';
								$is_link = true;
						   }
					   }
					...
					if($is_link){
						$cell = '<a href="'.$CFG->wwwroot.'/calendar/view.php?view=day&cal_d='.$day.'&cal_m='.$m.'&cal_y='.$y.'">'.$day.'</a>';
					}else{
						$cell ='<a href="'.$CFG->wwwroot.'/calendar/view.php?view=day&cal_d='.$day.'&cal_m='.$m.'&cal_y='.$y.'">';
						if(date('d') == $day && date('m') == $m && date('Y') == $y){ $cell .= '<div style="background-color:#fff;color:#000;font-weight:bold;">'; }
						$cell .= $day;
						if(date('d') == $day && date('m') == $m && date('Y') == $y){ $cell .= '</div>'; }
						$cell .= '</a>';
					}
			}
        	//elseif(is_centeradmin())
			else {
			$group_act = get_user_group_by_user();
				if($group_act){
					foreach($group_act as $group){
						$class_events = $DB->get_records('class_activity',array('groupid'=>$group->id));		
						$calendardate = strtotime($day.'-'.$m.'-'.$y.' 00:00:00');
						$calenderenddate= strtotime($day.'-'.$m.'-'.$y.' 23:59:59');
						$is_link = false;
						foreach($class_events as $class_event){
						...
						if($_SESSION['user_role'] == 'centeradmin' || is_institute_cordinator()){
							
							if($is_link){
							$cell = '<a href="'.$CFG->wwwroot.'/index_centeradmin.php?view=day&cal_d='.$day.'&cal_m='.$m.'&cal_y='.$y.'">'.$day.'</a>';
								}else{					    
							$cell ='<a href="'.$CFG->wwwroot.'/index_centeradmin.php?view=day&cal_d='.$day.'&cal_m='.$m.'&cal_y='.$y.'">';
							if(date('d') == $day && date('m') == $m && date('Y') == $y){ $cell .= '<div style="background-color:#fff;color:#000;font-weight:bold;">'; }
							$cell .= $day;
							if(date('d') == $day && date('m') == $m && date('Y') == $y){ $cell .= '</div>'; }
							$cell .= '</a>';
...
function calendar_top_controls($type, $data) {
    ...
    switch($type) {
        case 'frontpage':
            ...
            $nextlink = calendar_get_link_next(get_string('monthnext', 'access'), '?', 0, $nextmonth, $nextyear, $accesshide=true);
            $prevlink = calendar_get_link_previous(get_string('monthprev', 'access'), '?', 0, $prevmonth, $prevyear, true);
            ...
            $content .= html_writer::tag('span', userdate($time, get_string('strftimemonthyear')), array('title'=>get_string('monththis','calendar'),'class'=>'current'));
        ...
        case 'course':
            list($prevmonth, $prevyear) = calendar_sub_month($data['m'], $data['y']);
            list($nextmonth, $nextyear) = calendar_add_month($data['m'], $data['y']);
            $nextlink = calendar_get_link_next(get_string('monthnext', 'access'), '?id='.$data['id'].'&amp;', 0, $nextmonth, $nextyear, $accesshide=true);
            $prevlink = calendar_get_link_previous(get_string('monthprev', 'access'), '?id='.$data['id'].'&amp;', 0, $prevmonth, $prevyear, true);
        ...
        case 'display':
            ...
            $calendarlink= html_writer::tag('span', userdate($time, get_string('strftimemonthyear')), array('title'=>get_string('monththis','calendar'),'class'=>'current'));
            //$calendarlink = html_writer::link($calendarlink, userdate($time, get_string('strftimemonthyear')));
        ...
        case 'month':
            ...
            $prevlink = calendar_get_link_previous(userdate($prevdate, get_string('strftimemonthyear')), '?view=month'.$courseid.'&amp;', 1, $prevmonth, $prevyear);
            $nextlink = calendar_get_link_next(userdate($nextdate, get_string('strftimemonthyear')), '?view=month'.$courseid.'&amp;', 1, $nextmonth, $nextyear);
        ...
        case 'day':
			...
			if(is_teacher() || is_non_editing_teacher()){
				$page_to_show = 'index_teacher.php';
			}else{
				$page_to_show = 'calendar/view.php';
			}
            $prevlink = calendar_get_link_previous($prevname, $CFG->wwwroot.'/'.$page_to_show.'?view=day'.$courseid.'&amp;', $prevdate['mday'], $prevdate['mon'], $prevdate['year']);
            $nextlink = calendar_get_link_next($nextname, $CFG->wwwroot.'/'.$page_to_show.'?view=day'.$courseid.'&amp;', $nextdate['mday'], $nextdate['mon'], $nextdate['year']);
        ...
        case 'day2':
            ...
            $prevlink = calendar_get_link_previous($prevname, $CFG->wwwroot.'/'.$page_to_show.'?view=day2'.$courseid.'&amp;', $prevdate['mday'], $prevdate['mon'], $prevdate['year']);
            $nextlink = calendar_get_link_next($nextname, $CFG->wwwroot.'/'.$page_to_show.'?view=day2'.$courseid.'&amp;', $nextdate['mday'], $nextdate['mon'], $nextdate['year']);
...
/* A function to get the class name for the selected activity
 * 
 */

function get_classname_by_course_activity($activity,$course)
{
