...
function message_print_participants($context, $courseid, $contactselecturl=null, $showactionlinks=true, $titletodisplay=null, $page=0, $user2=null) {
    ...
    foreach ($participants as $participant) {
        if ($participant->id != $USER->id && !user_has_role_assignment($participant->id, 5) && !user_has_role_assignment($participant->id,10) && !is_siteadmin($participant->id)) {
...
function message_search_users($courseid, $searchtext, $sort='', $exceptions='') {
    ...
    if(!is_siteadmin())
    {
    	$siteser = ' AND u.site_fk = '.$USER->site_fk.'';
    }
    else {
    	$siteser = '';
    }
...
function message_mark_messages_read($touserid, $fromuserid){
    ...
    $sql = 'SELECT m.* FROM {message} m WHERE m.useridto=:useridto AND m.useridfrom=:useridfrom ORDER BY m.timecreated';
