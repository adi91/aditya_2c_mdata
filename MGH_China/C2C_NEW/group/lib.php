...
function groups_delete_group($grouporid) {
    ...
    // delete class for group
    $DB->delete_records('class_activity', array('groupid'=>$groupid));
    //group itself last
    $DB->delete_records('class_schedule', array('groupid'=>$groupid));
    //group itself last
...
function groups_get_groupby_role($userid,$courseid)
{
	global $DB;
	$groupssql = "SELECT g.* FROM {groups} g 
					LEFT JOIN {groups_members} gm ON gm.groupid = g.id
	 				WHERE g.courseid=? AND gm.userid = ?";
	 $param = array($courseid,$userid);
	 $rs = $DB->get_record_sql($groupssql, $param);
	 return $rs;
}

function get_group_activity($gropid,$sectionid,$course)
{
	global $DB;
	$res = $DB->get_record('class_activity',array('groupid'=>$gropid,'sectionno'=>$sectionid,'course'=>$course));
	return $res;
}

function get_group_les_activity($gropid,$sectionid,$course)
{
	global $DB;
	$res = $DB->get_record('class_activity',array('groupid'=>$gropid,'section'=>$sectionid,'course'=>$course));
	return $res;
}

function get_group_les_sin_activity($gropid,$sectionid,$course,$module)
{
	global $DB;
	$res = $DB->get_record('class_activity',array('groupid'=>$gropid,'section'=>$sectionid,'course'=>$course));
	return $res;
...
function get_group_act_no($gropid,$course)
{
	global $DB;
	$res = $DB->count_records('class_activity',array('groupid'=>$gropid,'course'=>$course));
	return $res;
}

/*
function get_user_group_by_user($userid)
{
	global $DB,$USER;
	if(is_parent()){
	$groupssql = "SELECT g.* FROM {groups} g 
					LEFT JOIN {groups_members} gm ON gm.groupid = g.id
	 				WHERE gm.userid IN (
	 				SELECT u.id AS userid FROM {user} AS u 
					LEFT JOIN {user} AS parent ON u.username = parent.parentlist WHERE u.parentlist LIKE '".$USER->username."')";
		$param = array($USER->id);	
	}
	else
	{
	...
	$param = array($USER->id);	
	 ...
	 $rs = $DB->get_records_sql($groupssql, $param);
	return $rs;
}*/
	
function get_user_group_by_user()
{
	global $DB,$USER,$CFG;
	if(is_parent()){
	$groupssql = "SELECT g.* FROM {groups} g 
			...
			$category_list = implode($category_list,",");
				...
				if($category_list == "")
				{
				   ...
				   $groupssql ="SELECT g.* FROM {groups} g 
						   		...
						   		WHERE c.category IN ($category_list)";
				}
...
function get_centeradmin_class_view($courseid)
{
global $DB;
$sql = "SELECT g.name FROM {groups} g  
			LEFT JOIN {class_schedule} cs ON cs.groupid=g.id 
			WHERE g.courseid=? AND cs.id IS NULL ";	
