...
function quiz_update_events($quiz, $override = null) {
    ...
    $sectionTime= $DB->get_record('course_sections',array('course'=>$assignment->course,'section'=>$assignment->section),'startdate');

    if($quiz->timeopen!=0)
    {
    	$timeQuiz= $sectionTime->startdate;
    }
    else
    {
    	$timeQuiz=$quiz->timeopen;
    }
    
    foreach ($overrides as $current) {
        $groupid   = isset($current->groupid)?  $current->groupid : 0;
        $userid    = isset($current->userid)? $current->userid : 0;
        $timeopen  = isset($current->timeopen)?  $current->timeopen : $timeQuiz;
