...
if ($formdata = $mform2->is_cancelled()) {
    ...
    while ($line = $cir->next()) {
        ...
        foreach ($line as $keynum => $value) {
			...
			if($key == 'site_fk'){
				$invalid_site = implode("*",array_diff(explode("*",$USER->site_fk),explode("*",$value)));
				$valid_site = implode("*",array_intersect(explode("*",$USER->site_fk),explode("*",$value)));
				if(is_siteadmin()){
					$value = $value.'-';
				}else{
					$value = $valid_site.'-'.$invalid_site;
				}
            ...
            if (strpos($key, 'profile_field_') === 0) {
                ...
                if (isset($USER->$key) and is_array($USER->$key)) {
					...
					//echo '<pre>';print_r($user->$key);echo '</pre>';
                } else {
                    $user->$key = $value;
					//echo '<pre>';print_r($user->$key);echo '</pre>';
                }
        ...
        /**********  added by Swapnil dt. 9 May 2012 **** Site admin site_fk is added to user site_fk *************/ 
		/*global $USER;
		if(!is_siteadmin())
		{
			$user->site_fk = $USER->site_fk;
		}*/
		
		/**********  added by Swapnil dt. 9 May 2012 **** Site admin site_fk is added to user site_fk *************/ 
			
			/* Code added by chandra to check if user with valid site_fk filed are updated else continue with the loop  */
			$centers = array();
			$authorize_center = array();
			$unauthorize_center = array();
			
			$centers = explode("-",$user->site_fk);		
			if($centers[0]){
				$authorize_center = explode("*",$centers[0]);
			}
			if($centers[1]){
				$unauthorize_center = explode("*",$centers[1]);
			}
			if(count($authorize_center) == 0){
				continue;
			}
			/*echo '<pre>Authorize Center :';print_r($authorize_center);echo '</pre>';
			echo '<pre>Unauthorize Center :';print_r($unauthorize_center);echo '</pre>';
			echo '<pre>count Authorize Center :';print_r(count($authorize_center));echo '</pre>';
			echo '<pre>Count Unauthorize Center :';print_r(count($unauthorize_center));echo '</pre>';
			*/
			$user->site_fk = implode("*",$authorize_center);
        ...
        if ($existinguser = $DB->get_record('user', array('username'=>$user->username, 'mnethostid'=>$CFG->mnet_localhost_id))) {
			/* If user who is uploading users from csv is site admin then any value in site_fk filed given in CSV will overrride already existing values but if the user is center admin then it will add the site_fk values to existing values so that user in others center are not disturbed  by other centeradmin */
			if(is_siteadmin()){
				$user->site_fk = $user->site_fk;
			}else{
				$current_site_fk = explode("*",$existinguser->site_fk);
				$new_site_fk = explode("*",$user->site_fk);
				$user->site_fk = implode("*",array_unique(array_merge($new_site_fk,$current_site_fk)));
       ...
       /* Start Swapnil
        * 27_3_12 
        To add the system role for participant and facilitator
        */
       /* foreach ($filecolumns as $column) {
        	$roleid ="";
            if ($column!='roleassign') {
                continue;
            }
		
            if (!empty($user->$column)) {
                $assignuser = $user->$column;
               	$systemcontext = get_context_instance(CONTEXT_SYSTEM);
            	if($assignuser==PARENTUSER || $assignuser =="PA")
            	{
            		$roleid = PARENTUSER;
            	}
            	if($assignuser==STUDENT || $assignuser =="ST")
            	{
            		$roleid = STUDENT;
            	}
            	if($roleid!="")
        ...
        foreach ($filecolumns as $column) {
            ...
            if (!array_key_exists($shortname, $ccache)) {
                if (!$course = $DB->get_record('course', array('shortname'=>$shortname), 'id, shortname, category')) {
			...
			if (!$center = $DB->get_record('sites', array('site_centerid'=>$ccache[$shortname]->category), 'id, site_name')) {
                    $upt->track('enrolments', get_string('unknowncourse', 'error', s($shortname)), 'error');
                    continue;
                }
			//echo  'course is in center id '.$center->id.'<br>';
			//echo  'user is in center id '.$user->site_fk.'<br>';
			//echo  'uploader is in centers id '.$USER->site_fk.'<br>';
			if(is_siteadmin()){
				if(!in_array($center->id,explode("*",$user->site_fk))){
					$upt->track('enrolments', get_string('usernotincoursecenter', 'error', s($shortname)), 'error');
					continue;
				}
			}else{
				if(!in_array($center->id,explode("*",$USER->site_fk))){
					$upt->track('enrolments', get_string('usernotallowedtoenroluserstothiscourse', 'error', s($shortname)), 'error');
					continue;
				}elseif(!in_array($center->id,explode("*",$user->site_fk))){
					$upt->track('enrolments', get_string('usernotincoursecenter', 'error', s($shortname)), 'error');
					continue;
				}
			}
            ...
            if ($manual and $manualcache[$courseid]) {
                ...
                if (!empty($user->{'role'.$i})) {
                    $addrole = $user->{'role'.$i};
	               /* if($addrole==PARENTUSER || $addrole =="PA")
	            	{
	            		$roleid = PARENTUSER;
	            	}
	            	if($addrole==STUDENT || $addrole =="ST")
	            	{
	            		$roleid = STUDENT;
	            	}*/
                    if (array_key_exists($roleid, $rolecache)) {
                        $rid = $rolecache[$roleid]->id;
                    } else {
    ...
    if ($bulk) {
        echo $OUTPUT->continue_button2($bulknurl);
    } else {
        echo $OUTPUT->continue_button2($returnurl);
    }
    echo $OUTPUT->footer();
    die;
}
