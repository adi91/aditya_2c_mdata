...
class admin_uploaduser_form1 extends moodleform {
    function definition () {
       ...
       // if (array_key_exists('cfg', $choices)) {
		...
		$mform->addElement('hidden','previewrows',100000);
...
class admin_uploaduser_form2 extends moodleform {
    function definition () {
            ...
            if ($studentroles = get_archetype_roles('student')) {
                foreach ($studentroles as $role) {
                    if (isset($choices[$role->id])) {
						...
						$mform->addElement('hidden','uulegacy1',$role->id);
                        break;
                    }
                }
                unset($studentroles);
            ...
            if ($editteacherroles = get_archetype_roles('editingteacher')) {
                foreach ($editteacherroles as $role) {
                    if (isset($choices[$role->id])) {
						...
						$mform->addElement('hidden','uulegacy2',$role->id);
                        break;
                    }
                }
            ...
            if ($teacherroles = get_archetype_roles('teacher')) {
                foreach ($teacherroles as $role) {
                    if (isset($choices[$role->id])) {
						...
						$mform->addElement('hidden','uulegacy3',$role->id);
                        break;
                    }
                }
                unset($teacherroles);
        ...
        if (empty($CFG->enableajax)) {
			...
			$mform->addElement('hidden','ajax', 1);
        }
        ...
        if (empty($CFG->defaultcity)) {
			...
			$mform->addElement('hidden','city', $templateuser->city);
        } else {
        ...
        if (empty($CFG->country)) {
			...
			$mform->addElement('hidden','country', $templateuser->country);
        } else {
       ...
       // $choices['99'] = get_string('serverlocaltime');
		...
		$mform->addElement('hidden','timezone', $templateuser->timezone);
		...
		$mform->addElement('hidden','institution', $templateuser->institution);
