<?php
	...
	//echo '<pre>';print_r($extrasql);echo '</pre>';
    ...
    if (!$users) {
       ...
       /* $table->head[] = $city;
        ...
        foreach ($users as $user) {
            ...
            if($user->parentlist!="")
            {
            	$parentlist= wordwrap(str_replace("*","</br>",$user->parentlist),"13","</br>",true);
            }
            $fullname = fullname($user, true);
            ...
            $site_names= get_site_name_by_id($user->site_fk);
			$centers = "";
			$count = 0;
			foreach($site_names as $site_name){
			     $centers .= $site_name->site_name;
				 if(count($site_names)>1 && $count < (count($site_names)-1)){
				   $centers .= ", ";
				 }
				 $count++;
			}
			...
			$row[]= $centers;
            if($CFG->theme == 'mghc2c'){}else{$row[] = $parentlist;}
            if(is_siteadmin() || is_mhescordinator()){
            $row[]= $strlastaccess;
            }
            if ($user->suspended) {
    ...
    if (has_capability('moodle/user:create', $sitecontext)) {
        echo $OUTPUT->heading('<a href="'.$securewwwroot.'/user/editadvanced.php?id=-1" class="buttonstyled">'.get_string('addnewuser').'</a>');
    }
    if (!empty($table)) {
