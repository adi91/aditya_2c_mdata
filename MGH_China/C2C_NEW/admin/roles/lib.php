...
class potential_assignees_course_and_above extends role_assign_user_selector_base {
    public function find_users($search) {
		...
		$context = context::instance_by_id($this->context->id);
		
		$center = $DB->get_record('sites',array('site_centerid'=>$context->instanceid));
        ...
        $sql = " FROM {user}
				...
				if($this->context->id != 1){
					$sql .= " AND (site_fk like '%*".$site_fk."' OR site_fk like '".$site_fk."' OR site_fk like '".$site_fk."*%' OR site_fk like '%*".$site_fk."*%' )  ";
				}
				$sql .= " AND id NOT IN (
                         SELECT r.userid
                           FROM {role_assignments} r
