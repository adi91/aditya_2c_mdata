            ...
            if (has_capability('moodle/course:create', $categorycontext)) {
				...
				if(is_centeradmin()){
					...
					foreach($categories as $category){ 
						 ...
						 if($check_role_assignment->id != ''){
						    $displaylist[$category->id] = $category->name;
						 }
					}
					...
					if($check_role_assignments){			
						foreach($check_role_assignments as $check_role_assignment){	
							$displaylist[$check_role_assignment->id] = $check_role_assignment->name;
						}
					}
				}elseif(is_mhescordinator()){
					$categories = get_categories();
					$displaylist = array();
					foreach($categories as $category){ 
						// $category_context =  context_coursecat::instance($category->id);
...
//--------------------------------------------------------------------------------
	...
	if (empty($course->id)) 
	{
       $this->add_action_buttons(true,get_string("createcourse"));
	}
	else
	{
