...
if (!$courses) {
    ...
    foreach ($courses as $acourse) {
		...
		if(is_siteadmin() || is_mhescordinator()){
			echo '<td><a '.$linkcss.' href="view.php?catid='.$category->id.'&id='.$acourse->id.'">'. format_string($coursename) .'</a></td>';
		}else{
			echo '<td><a '.$linkcss.' href="courset.php?id='.$acourse->id.'">'. format_string($coursename) .'</a></td>';
		}
        if ($editingon) {
            echo '<td>';
