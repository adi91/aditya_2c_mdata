...
require_once('editcenter_form.php');
...
if ($id) {
    ...
    $PAGE->set_url('/course/editcenter.php', array('id' => $id));
	...
	$parent = 0;
    $PAGE->set_url('/course/editcenter.php', array('parent' => $parent));
    if ($parent) {
    ...
    $strtitle = get_string("addnewcenter");
...
$mform = new editcenter_form('editcenter.php', compact('category', 'editoroptions'));
$mform->set_data($category);

if ($mform->is_cancelled()) {
    if ($id) {
        redirect($CFG->wwwroot . '/course/center.php?id=' . $id . '&categoryedit=on');
    } else if ($parent) {
        redirect($CFG->wwwroot .'/course/center.php?id=' . $parent . '&categoryedit=on');
    } else {
        redirect($CFG->wwwroot .'/course/center.php?categoryedit=on');
    }
	...
	$site = $DB->get_record('sites',array('site_centerid'=>$newcategory->id));
	
    if($site->id){
		$data_center = new stdClass();
		$data_center->id = $site->id;
		$data_center->site_id = $newcategory->idnumber;
		$data_center->site_name = $newcategory->name;
		$data_center->site_desc = $newcategory->description_editor;
		$data_center->site_author = $USER->firstname.' '.$USER->lastname;
		$data_center->site_created = time();
		$data_center->site_modified = time();
		$data_center->site_centerid = $newcategory->id;
		$DB->update_record('sites',$data_center);
	}else{
		$data_center = new stdClass();
		$data_center->id = 0;
		$data_center->site_id = $newcategory->idnumber;
		...
		$data_center->site_modified = time();
		$data_center->site_centerid = $newcategory->id;
		$DB->insert_record('sites',$data_center);
	}  

    redirect('center.php?id='.$newcategory->id.'&categoryedit=on');
}
