...
function get_print_section_cm_text(cm_info $cm, $course) {
    ...
    if (($content = $cm->get_content()) !== '') {
        ...
        $labelformatoptions->noclean = false;
        $labelformatoptions->overflowdiv = false;
...
function print_section($course, $section, $mods, $modnamesused, $absolute=false, $width="100%", $hidecompletion=false) {
	...
	$group= groups_get_groupby_role($USER->id,$course->id);
    $tl = textlib_get_instance();
    ...
    if (!empty($section->sequence)) {
		...
		$sec_act=get_group_les_activity($group->id,$section->id,$course->id);
        foreach ($sectionmods as $modnumber) {
            ...
            if ($mod->uservisible) {
				...
				if(is_student() && $sec_act->availablefrom > $endday ){
					$linkclasses = ' dimmed';
                    $textclasses = ' dimmed_text';
					$linkcss = 'class="' . trim($linkclasses) . '" ';
				}
                ...
                if($mod->icon=="f/pdf")
                {	
                	$resource = $DB->get_record("resource", array("id"=>$mod->instance));
                	$fs = get_file_storage();
					$files = $fs->get_area_files($mod->context->id, 'mod_resource', 'content', 0, 'sortorder');
					$file = array_pop($files);
					$path = '/'.$mod->context->id.'/mod_resource/content/'.$resource->revision.$file->get_filepath().$file->get_filename();
					$fullurl = file_encode_url($CFG->wwwroot.'/pluginfile.php', $path, false);
					//echo $fullurl;
					if(is_student() && $sec_act->availablefrom >= $startday && $sec_act->availablefrom < $endday){
                	$onclick = "onclick='window.open(\"$fullurl\",\"\",\"width=1000,height=900,toolbar=no,location=no,menubar=no,copyhistory=no,status=no,directories=no,scrollbars=yes,resizable=yes\");'";
					}
					
					
                }
                else if($mod->icon=="f/web")
                {
                	///$resource = $DB->get_record("resource", array("id"=>$mod->instance));
                	//$url = $DB->get_record("url", array("id"=>$mod->instance));
                	$path=$CFG->wwwroot."/mod/url/view.php?id=".$mod->id;
 					$fullurl = file_encode_url($CFG->wwwroot.'/pluginfile.php', $path, false);
					//echo $fullurl;
					if(is_student() &&$sec_act->availablefrom >= $startday && $sec_act->availablefrom < $endday){
                	$onclick = "onclick='window.open(\"$path\",\"\",\"width=1000,height=900,toolbar=no,location=no,menubar=no,copyhistory=no,status=no,directories=no,scrollbars=no,resizable=yes\");'";
					}
				...
				if($mod->icon=="f/pdf") {	
	           		 echo '<a ' . $linkcss . $mod->extra . $onclick .
	                            ...
	                            $modulename . '" style="cursor:pointer;" /> ' .
	                            $accesstext . '<span class="instancename '.$linkclasses.'" style="cursor:pointer;">' .
				...
				else
				{
				   ...
				   {
					$url="#";
					$linkcss2 =" dimmed_text";
				   }
	                    // Display link itself
	                    echo '<a ' . $linkcss . $mod->extra . $onclick .
	                            ' href="' . $url . '" . '.$target.'  ><img src="' . $mod->get_icon_url() .
	                            '" class="activityicon" alt="' .
	                            $modulename . '" /> ' .
	                            $accesstext . '<span class="instancename '.$linkcss2.'">' .
	                            $instancename . $altname . '</span></a>';
...
function print_section_new($course, $section, $mods, $modnamesused, $absolute=false, $width="100%", $hidecompletion=false) {
    global $CFG, $USER, $DB, $PAGE, $OUTPUT;
    ...
    if (!empty($section->sequence)) {
        ...
        // Fix bug #5027, don't want style=\"width:$width\".
        //echo "<ul class=\"section img-text\">\n";
      echo "<div class='index-rightboard'>";
      echo "<div class='sectionlesson'>";
        $sectionmods = explode(",", $section->sequence);
        ...
        foreach ($sectionmods as $modnumber) {
            ...
            $modulename = $modulenames[$mod->modname];
 			...
 			//echo $introtext = format_module_intro($modulename,15,13);    
            // In some cases the activity is visible to user, but it is
            // dimmed. This is done if viewhiddenactivities is true and if:
            // 1. the activity is not visible, or
            // 2. the activity has dates set which do not include current, or
          ...
          //  echo html_writer::start_tag('li', array('class'=>join(' ', $liclasses), 'id'=>'module-'.$modnumber));
        //  echo html_writer::start_tag('div', array('class'=>'box1wf', 'id'=>'module-'.$modnumber));
        //  echo html_writer::start_tag('div', array('class'=>'le-number-gr'));
        //  echo html_writer::end_tag('div');
           /* if ($ismoving) {
                echo '<a title="'.$strmovefull.'"'.
                     ' href="'.$CFG->wwwroot.'/course/mod.php?moveto='.$mod->id.'&amp;sesskey='.sesskey().'">'.
           ...
           /* if ($mod->uservisible) {
            ...
            /*if ($url = $mod->get_url()) {
                    // Display link itself
                    echo '<h4 class="hd1">'.$instancename.'</h4><span class="instancename">' .
                            $altname . '</span>';
...
*/
        ...
        //  echo html_writer::end_tag('div');
            ...
            echo "<div class='box1wf'>";
            	echo "<div class='le-number-gr'>";
                echo "<span>".$cntLess."</span>";
                echo "</div>";
             echo "<div class='le-details'>";
                 echo "<div class='task-le'>";
                	echo "<div class='table'>";
                     echo "<div style='display:block'><h4 class='hd1'>".$instancename."</h4>";
                      echo "<span class='pr1g'>Score</span></div>";
                      echo "<div class='pr1'>".$content."</div>";
                        echo "<div class='tableaside'><div class='calc'>92%</div><div class='yel-bar'><div class='gre-bar' style='width:92%;'></div></div></div>";
                    echo "</div>";
				echo "</div>";
                echo "<div class='taskcomplete-le'>";
                echo "<div class='pr1gl'>";
                  echo "<span>25 May 2012 - 14:00</span>";
                        echo "<div class='right'>";
                        echo "<span class='right'>2 TasKs COMPLETED</span><br/>";
...
function print_section_tec($course, $section, $mods, $modnamesused, $absolute=false, $width="100%", $hidecompletion=false) {
    global $CFG, $USER, $DB, $PAGE, $OUTPUT;
    ...
    if (!empty($section->sequence)) {
        ...
        //  echo html_writer::end_tag('div');
          ...
          /*  } else {
           ...
           echo '<div class="actbox1wh">
            	<div class="arr"></div>
            	<div class="actbox-number-gr2">
                </div>
                 <div class="le-details2">
                 <div class="task-le">
                	<div class="table">
                        <div style="display:block"><h4 class="hd1"><a href='.$url.' '.$onclick.' class="hd1" '.$target.'>'.$instancename.'</a></h4>
                        </div>
                        <div class="pr2">&nbsp;'.$content.'</div>
                    </div>
				</div>';
           if(is_teacher() || is_centeradmin()){
                echo '<div class="taskcomplete-le">
                	<div class="pr1gl">
                                         
                        <div class="right">
                        <span class="right">';
                           echo make_editing_buttons_tec($mod, $absolute, true, $mod->indent, $section->section);
                        
	                       /*echo '<span class="gray_tick" title="ok"></span>
	                        <span class="red_view" title="View"></span>
	                        <span class="gray_edit"  title="Edit"></span>
	                        <span class="green_info" title="Info"></span> 
	                        <span class="green_pro" title="Profile"></span> 
	                        <span class="gray_list" title="List"></span> 
	                        <span class="gray_mark" title="mark"></span>
	                        <span class="gray_duplicate" title="Duplicate">
	                        </span><span class="gray_delete" title="Delete">
                        </span>*/
...
function print_section_add_menus_resouce($course, $section, $modnames, $vertical=false, $return=false) {
    global $CFG, $OUTPUT;
    ...
    $output  = '<div class="section_add_menus_res" id="res_'.$section.'"  style="display:none;">';
    ...
    if (!empty($resources)) {
        $select = new url_select($resources, '', array(''=>$straddresource), "ressection$section","Go");
       // $select->set_help_icon('resources');
        $output .= $OUTPUT->render($select);
    }
    if (!$vertical) {
...
function print_section_add_activities($course, $section, $modnames, $vertical=false, $return=false) {
    global $CFG, $OUTPUT;
    ...
    $straddactivity = get_string('addactivity');
    $output  = '<div class="section_add_menus_act" id="act_'.$section.'" style="display:none;">';

    if (!$vertical) {
        $output .= '<div class="horizontal">';
    }
    if (!empty($activities)) {
        $select = new url_select($activities, '', array(''=>$straddactivity), "section$section" ,"Go");
       // $select->set_help_icon('activities');
        $output .= $OUTPUT->render($select);
...
function make_categories_list(&$list, &$parents, $requiredcapability = '',
    ...
    if (empty($category)) {
        ...
        /* Swapnil changes for Category for user
         * This condition is added to check user course category
         * 
         */
     if(!is_siteadmin() && $USER->institution == $category->id)
     {
...
function get_course_display_desc_for_list($course) {
    global $CFG;
...
function print_course_profile($course, $highlightterms = '') {
    global $CFG, $USER, $DB, $OUTPUT;
    ...
    // Rewrite file URLs so that they are correct
   $course->summary = file_rewrite_pluginfile_urls($course->summary, 'pluginfile.php', $context->id, 'course', 'summary', NULL);
	echo html_writer::start_tag('div', array('class'=>'sectionlesson'));
    echo html_writer::start_tag('div', array('class'=>'box1wf'));
    	 echo html_writer::start_tag('div', array('class'=>'le-details'));
     		echo html_writer::start_tag('div', array('class'=>'task-le'));
	     		echo html_writer::start_tag('div', array('class'=>'table'));
		     		echo html_writer::start_tag('div', array('style'=>'display:block'));
		     				echo html_writer::start_tag('h4', array('class'=>'hd1'));
		     				$linkhref = new moodle_url('/course/courselesson.php', array('id'=>$course->id));
		     				$coursename = get_course_display_name_for_list($course);
						    $linktext = highlight($highlightterms, format_string($coursename));
						    $linkparams = array('title'=>get_string('entercourse'));
						    echo html_writer::link($linkhref, $linktext, $linkparams);
		     			echo html_writer::end_tag('h4'); //hd1
		     			
		     			echo html_writer::start_tag('div', array('class'=>'pr1'));
		     				echo $coursedesc = get_course_display_desc_for_list($course);
		     			echo html_writer::end_tag('div'); //pr1
		   	 		echo html_writer::end_tag('div');//div
	   	 		echo html_writer::end_tag('div'); //table
    	 	echo html_writer::end_tag('div');  // task-le
   	 	echo html_writer::end_tag('div'); //le-datails
    echo html_writer::end_tag('div'); 
    echo html_writer::end_tag('div'); 
   ...
   /* echo html_writer::start_tag('div', array('class'=>'coursebox clearfix'));
    echo html_writer::start_tag('div', array('class'=>'info'));
    echo html_writer::start_tag('h4', array('class'=>'name'));
    $linkhref = new moodle_url('/course/view.php', array('id'=>$course->id));
    $coursename = get_course_display_name_for_list($course);
    $linktext = highlight($highlightterms, format_string($coursename));
    $linkparams = array('title'=>get_string('entercourse'));
    ...
    echo html_writer::start_tag('div', array('class'=>'pr1'));
    $coursedesc = get_course_display_desc_for_list($course);
     echo html_writer::end_tag('div');
*/
    /// first find all roles that are supposed to be displayed
    if (!empty($CFG->coursecontact)) {
            		...
            		echo html_writer::start_tag('div', array('class'=>'topbx'));
            			echo html_writer::start_tag('div', array('class'=>'topbx'));
							...
							echo html_writer::start_tag('div', array('class'=>'topbx-ri'));
                				echo $name;
                				/*echo "<p class='pr1'>Nationality: British<br />
...
function print_course_profile_tech($course, $highlightterms = '') {
    global $CFG, $USER, $DB, $OUTPUT;
...
*/
    ...
    if (!empty($CFG->coursecontact)) {
        ...
        if (!empty($namesarray)) {
            echo html_writer::start_tag('div', array('class'=>'sectionmessage'));
            echo html_writer::start_tag('h4');
            echo $coursename ."". get_string('teacherare');
            echo html_writer::start_tag('h4');
           $cnt=1;
            foreach ($namesarray as $key => $name) {
           /// print_r($key ."****". $name);
...
function print_my_moodle_profile() {
    global $USER, $CFG, $DB, $OUTPUT;
    ...
    $courses  = apt_enrol_get_users_courses($USER->id, true, NULL, 'visible DESC,sortorder ASC'); //enrol_get_my_courses('summary', 'visible DESC,sortorder ASC');
    $rhosts   = array();
    $rcourses = array();
    if (!empty($CFG->mnet_dispatcher_mode) && $CFG->mnet_dispatcher_mode==='strict') {
        $rcourses = get_my_remotecourses($USER->id);
...
function make_editing_buttons_tec(stdClass $mod, $absolute_ignored = true, $moveselect = true, $indent=-1, $section=-1) {
	...
	/*Added by Pankaj - for section move up/down*/
    global $DB;
	if ($hasmanageactivities) 

            {

                        $section_list = $DB->get_record("course_sections", array("course"=>$mod->course, "section"=>$section));            

                        $arr_mod = explode(",",$section_list->sequence);

                        if(array_search($mod->id,$arr_mod) > 0)

                        {

                  $actions[] = new action_link(

                new moodle_url('#', array()),

                new pix_icon('t/up', $str->moveup, 'moodle', array('class' => 'iconsmall')),

                null,

                array('class' => 'editing_moveup', 'id' => $mod->id."_".$mod->course.'_'.$section,'title' => $str->moveup,'onclick'=>'return activity_move_up(this);')

            );

                        }           

                        if(array_search($mod->id,$arr_mod) < (count($arr_mod) -1))
            ...
            $actions[] = new action_link(
                ...
                array('class' => 'editing_movedown', 'id' => $mod->id."_".$mod->course.'_'.$section,'title' => $str->movedown,'onclick'=>'return activity_move_down(this);')
	...
	/*Added by Pankaj - for section move up/down -- END --*/
    ...
    if ($hasmanageactivities) {
        $actions[] = new action_link(
            new moodle_url($baseurl, array('update' => $mod->id)),
           new pix_icon('t/m_edit', $str->update, 'moodle', array('class' => 'iconsmall17')),
            null,
            array('class' => 'gray_edit', 'title' => $str->update)
        );
    }
    ...
    if (has_all_capabilities($dupecaps, $coursecontext)) {
        $actions[] = new action_link(
            new moodle_url($baseurl, array('duplicate' => $mod->id)),
            new pix_icon('t/m_copy', $str->duplicate, 'moodle', array('class' => 'iconsmall17')),
            null,
            array('class' => 'gray_duplicate', 'title' => $str->duplicate)
        );
    }
    ...
    if ($hasmanageactivities) {
        $actions[] = new action_link(
            new moodle_url($baseurl, array('delete' => $mod->id)),
            new pix_icon('t/m_delete', $str->delete, 'moodle', array('class' => 'iconsmall17')),
            null,
            array('class' => 'gray_delete', 'title' => $str->delete)
        );
    }
    ...
    if (has_capability('moodle/course:activityvisibility', $modcontext)) {
        if ($mod->visible) {
            $actions[] = new action_link(
                new moodle_url($baseurl, array('hide' => $mod->id)),
                new pix_icon('t/m_view_green', $str->hide, 'moodle', array('class' => 'iconsmall17')),
                null,
                array('class' => 'red_view', 'title' => $str->hide)
            );
        } else {
            $actions[] = new action_link(
                new moodle_url($baseurl, array('show' => $mod->id)),
                new pix_icon('t/m_view', $str->show, 'moodle', array('class' => 'iconsmall17')),
                null,
                array('class' => 'gray_view', 'title' => $str->show)
            );
        }
    }
