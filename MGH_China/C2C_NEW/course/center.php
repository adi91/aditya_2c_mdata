...
$PAGE->set_url('/course/center.php');
...
if($CFG->theme='mghc2c'){
	$strcenters = get_string('institutes');
	$strcenter = get_string('institute');
}else{
	$strcenters = get_string('centers');
	$strcenter = get_string('center');
}
...
/// Print form for creating new categories
    ...
    if ($countcategories > 1 || ($countcategories == 1 && $DB->count_records('course') > 200)) {
        $strcourses = get_string('courses');
        $strcategories = get_string('centers');

        $PAGE->navbar->add($strcenters);
        $PAGE->set_title("$site->shortname: $strcenters");
        $PAGE->set_heading($COURSE->fullname);
        $PAGE->set_button(update_category_button());
        echo $OUTPUT->header();
        echo $OUTPUT->heading($strcenters);
...
if (!empty($delete) and confirm_sesskey()) {
    ...
    $heading = get_string('deletecenter', 'moodle', format_string($deletecat->name, true, array('context' => $context)));
    ...
    if ($data->fulldelete) {
		//print_r($deletecat->id);
		$DB->delete_records('sites',array('site_centerid'=>$deletecat->id));
		//die();
    ...
    echo $OUTPUT->continue_button2('center.php');
...
$strcentersid = get_string('centersid');
$strcenters = get_string('centers');
$strcourses = get_string('courses');
...
echo '<th class="header" scope="col">'.$strcentersid.'</th>';
echo '<th class="header" scope="col">'.$strcenters.'</th>';
...
if (has_capability('moodle/category:manage', $systemcontext)) {
    ...
    echo $OUTPUT->single_button(new moodle_url('editcenter.php', $options), get_string('addnewcenter'), 'get');
}
...
global $DB;
        echo '<tr>';
		echo '<td>';
		$center1 = $DB->get_record('sites',array('site_centerid'=>$category->id));
        ...
        echo '<td class="icons">';    /// Print little icons
	        ...
	        if (has_capability('moodle/category:manage', $category->context)) {
	            ...
	            echo '<a title="'.$str->edit.'" href="editcenter.php?id='.$category->id.'"><img'.
	                 ' src="'.$OUTPUT->pix_url('t/edit') . '" class="iconsmall" alt="'.$str->edit.'" /></a> ';
	
	            echo '<a title="'.$str->delete.'" href="center.php?delete='.$category->id.'&amp;sesskey='.sesskey().'"><img'.
	                 ' src="'.$OUTPUT->pix_url('t/delete') . '" class="iconsmall" alt="'.$str->delete.'" /></a> ';
	
	            if (!empty($category->visible)) {
	                echo '<a title="'.$str->hide.'" href="center.php?hide='.$category->id.'&amp;sesskey='.sesskey().'"><img'.
	                     ' src="'.$OUTPUT->pix_url('t/hide') . '" class="iconsmall" alt="'.$str->hide.'" /></a> ';
	            } else {
	                echo '<a title="'.$str->show.'" href="center.php?show='.$category->id.'&amp;sesskey='.sesskey().'"><img'.
	            ...
	            if ($up) {
	                echo '<a title="'.$str->moveup.'" href="center.php?moveup='.$category->id.'&amp;sesskey='.sesskey().'"><img'.
	                     ' src="'.$OUTPUT->pix_url('t/up') . '" class="iconsmall" alt="'.$str->moveup.'" /></a> ';
	            } else {
	                echo $str->spacer;
	            }
	            if ($down) {
	                echo '<a title="'.$str->movedown.'" href="center.php?movedown='.$category->id.'&amp;sesskey='.sesskey().'"><img'.
				...
				//echo '<a title="Assign center admin" href="'.$CFG->wwwroot.'/admin/roles/assign_centeradmin_role.php?contextid='.$categorycontext->id.'&roleid=10"><img'.
	                     //' src="'.$OUTPUT->pix_url('i/roles') . '" class="iconsmall" alt="'.$str->cohorts.'" /></a> ';
				$assigntitle = "Assign center admin";
				$roleid = 10;
				if($CFG->theme == 'mghc2c'){
					$assigntitle = "Assign institute coordinator";
					$roleid = 13;
				}
				echo '<a title="'.$assigntitle.'" href="'.$CFG->wwwroot.'/admin/roles/assign.php?contextid='.$categorycontext->id.'&roleid='.$roleid.'"><img'.
	                     ' src="'.$OUTPUT->pix_url('i/roles') . '" class="iconsmall" alt="'.$str->cohorts.'" /></a> ';
    ...
    if ($categories = get_categories($category->id)) {   // Print all the children recursively
        ...
        foreach ($categories as $cat) {
                if($cat->parent == 0){
		            ...
		            $first = false;
					if(!is_siteadmin() && !is_mhescordinator()  && $USER->institution == $cat->id){
		            print_category_edit($cat, $displaylist, $parentslist, 1, $up, $down);
					}
					else if(is_siteadmin() || is_mhescordinator())
					{
					print_category_edit($cat, $displaylist, $parentslist, 1, $up, $down);	
					}
				}
