...
<?php 
        ...
        if (!has_capability('moodle/course:viewhiddensections', $context) and !$thissection->visible) {   // Hidden for students
            ...
            if (!is_null($thissection->name)) {
                echo $OUTPUT->heading(format_string($thissection->name, true, array('context' => $context)), 3, 'sectionnameview');
            }else{
				echo $OUTPUT->heading(format_string('Lesson '.$thissection->section, true, array('context' => $context)), 3, 'sectionnameview');
			}
            echo '<div class="summaryview">';
            ...
            print_section($course, $thissection, $mods, $modnamesused);
			if($CFG->theme == 'mghc2c'){
			echo '<div id="assessreport_'.$thissection->id.'" style="display:none;">'.lesson_quiz_report($course->id,$thissection->id,14).'</div>';
            ?>
			<script>
				function showasses_report(id){
					var sectionid = id;
					if(document.getElementById('assessreport_'+sectionid).style.display == 'none'){
						document.getElementById('assessreport_'+sectionid).style.display = 'block';
					}else{
						document.getElementById('assessreport_'+sectionid).style.display = 'none';
					}
				}
			</script>
			<?php
