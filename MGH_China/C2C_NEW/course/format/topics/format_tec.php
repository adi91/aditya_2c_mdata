    ...
    if ($showsection) {
       ...
       // echo '<div class="content">';
        if (!has_capability('moodle/course:viewhiddensections', $context) and !$thissection->visible) {   // Hidden for students
            ...
            if (!is_null($thissection->name)) {
                $sec_hed = $OUTPUT->heading(format_string($thissection->name, true, array('context' => $context)), 3, 'sectionname');
            }
            else
            {
            	$sec_hed = "Lesson". $section;
            }
         //   echo '<div class="summary">';
            if ($thissection->summary) {
                ...
                $summaryformatoptions->noclean = true;
                $summaryformatoptions->overflowdiv = true;
                $sec_text= format_text($summarytext, $thissection->summaryformat, $summaryformatoptions);
            } else {
               $sec_text =format_text('&nbsp;',$thissection->summaryformat,$summaryformatoptions);
            }
           ...
           // echo '</div>';
             ...
             echo '<div class="sectionlessonborder">';
            echo '<div class="actbox1wf">
            	<div class="actbox-number-gr">
                </div>
                 <div class="le-details">
                 <div class="task-le">
                	<div class="table">
                        <div style="display:block"><h4 class="hd1">'.$sec_hed.'</h4>
                        </div>
                        <div class="pr2">'.$sec_text.'</div>
                    </div>
				</div>';
			if(is_teacher() || is_centeradmin()){
              echo '<div class="taskcomplete-le">';
               echo '<div class="pr1gl" id='.$section.'><span ><a  onclick="get_act('.$section.');" id="show2">
               <img src="../pix/add_activity.jpg" alt="" width="98" height="22" />';
               echo '  <a onclick="get_res('.$section.');"><img src="../pix/add_resource.jpg" alt="" width="98" height="22" /></a>';
               echo '</span>';
               echo '<div id= "cor_act_'.$section.'" class="left">'.print_section_add_activities($course, $section, $modnames).'</div>';
                echo '<div id= "cor_act_'.$section.'" class="left">'.print_section_add_menus_resouce($course, $section, $modnames).'</div>';
                echo '<div class="right">
                        <span class="right"><a title="'.$streditsummary.'" href="editsection.php?id='.$thissection->id.'">
                        <img src="'.$OUTPUT->pix_url('t/m_edit') . '" class="icon" alt="Edit" />
                        </a></span></div>
                        </div>
				</div>';
                echo '</a>';
            ...
            print_section_tec($course, $thissection, $mods, $modnamesused);
...
if (!empty($sectionmenu)) {
    $select = new single_select(new moodle_url('/course/courseviewtech.php', array('id'=>$course->id)), 'topic', $sectionmenu);
